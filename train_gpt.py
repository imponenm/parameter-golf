...
class Hyperparameters:
    ...
    eval_stride = int(os.environ.get("EVAL_STRIDE", 256))  # 0 = no overlap; 256 for sliding window eval
...
def build_ppmi_svd_vectors(
    ...
    """
    Build dense semantic vectors from token co-occurrence in training data.

    Steps:
      1. Scan training shards, count co-occurrences via bincount (fast).
      2. PPMI weighting (positive pointwise mutual information).
      3. Truncated SVD → dense vectors of size model_dim.

    Returns [vocab_size, model_dim] tensor, or None if no data found.
    """
    ...
    for shard_path in shard_files:
        ...
        for offset in range(1, window + 1):
            ...
            right = tokens[offset:]
            # Fast symmetric counting via bincount on flattened indices
            flat = left * V + right
    ...
    # Normalise to unit std so the mixing alpha is interpretable
    std = vectors.std()
    if std > 1e-6:
        vectors = vectors / std
...
def main() -> None:
    ...
    # -----------------------------
    # SEMANTIC EMBEDDING INIT + DATA LOADER & MODEL WARMUP
    # -----------------------------

    # Blend PPMI-SVD co-occurrence vectors into the token embedding weights.
    # This gives the embeddings a structured starting point (semantic neighbours
    # start nearby) instead of pure random init — zero runtime overhead.
        ...
        if svd_vecs is not None:
            with torch.no_grad():
                base_model.tok_emb.weight.add_(
                    svd_vecs.to(dtype=base_model.tok_emb.weight.dtype, device=device) * args.sem_embed_alpha
                )
            log0(f"sem_embed_init:alpha={args.sem_embed_alpha} built in {1000*(time.perf_counter()-t_sem):.0f}ms")
        else:
            log0("sem_embed_init:skipped (no training shards found)")
    ...
    if master_process:
        with open("final_model.int6.zst", "wb") as f:
            f.write(quant_blob)
        quant_file_bytes = os.path.getsize("final_model.int6.zst")
        code_bytes = len(code.encode("utf-8"))
